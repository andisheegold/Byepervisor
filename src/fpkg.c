...
void GenPfsCryptoKey(uint8_t* p_EncryptionKeyPFS, uint8_t p_Seed[PFS_SEED_SIZE], uint32_t p_Index, uint8_t p_Key[PFS_FINAL_KEY_SIZE])
{
...
void GenPfsEncKey(uint8_t* p_EncryptionKeyPFS, uint8_t p_Seed[PFS_SEED_SIZE], uint8_t p_Key[PFS_FINAL_KEY_SIZE])
{
    GenPfsCryptoKey(p_EncryptionKeyPFS, p_Seed, 1, p_Key);
}

void GenPfsSignKey(uint8_t* p_EncryptionKeyPFS, uint8_t p_Seed[PFS_SEED_SIZE], uint8_t p_Key[PFS_FINAL_KEY_SIZE])
{
    GenPfsCryptoKey(p_EncryptionKeyPFS, p_Seed, 2, p_Key);
}

int32_t DecryptNpdrmDebugRif(uint32_t p_Type, uint8_t* p_Data)
{
...
struct sbl_map_list_entry* SceSblDriverFindMappedPageListByGpuVa(vm_offset_t p_GpuVa)
{
    if (p_GpuVa == 0)
    {
...
vm_offset_t SceSblDriverGpuVaToCpuVa(vm_offset_t p_GpuVa, size_t* p_NumPageGroups)
{
    if (p_GpuVa == 0)
    {
...
int32_t OnSceSblDriverSendMsg(SblMsg* p_Message, size_t p_Size)
{
...
int32_t OnSceSblPfsSetKeys(uint32_t* ekh, uint32_t* skh, uint8_t* eekpfs, Ekc* eekc, uint32_t pubkey_ver, uint32_t key_ver, PfsHeader* hdr, size_t hdr_size, uint32_t type, uint32_t finalized, uint32_t is_disc)
{
	...
	if (ret) 
    {
        ...
        else 
        {
			...
			A_sx_xlock_hard(sbl_pfs_sx,0);
			{
				...
				{
					enc_key_desc.pfs.obfuscated_key_id = PFS_FAKE_OBF_KEY_ID;
					enc_key_desc.pfs.key_size = sizeof(enc_key_desc.pfs.escrowed_key);

					GenPfsEncKey(ekpfs, hdr->cryptSeed, enc_key_desc.pfs.escrowed_key);

					fpu_kern_enter(td, fpu_kern_ctx, 0);
					{
						memset(iv, 0, sizeof(iv));
						ret = AesCbcCfb128Encrypt(enc_key_desc.pfs.escrowed_key, enc_key_desc.pfs.escrowed_key, sizeof(enc_key_desc.pfs.escrowed_key), g_FakeKeySeed, sizeof(g_FakeKeySeed) * 8, iv);
					}
					fpu_kern_leave(td, fpu_kern_ctx);
				}
				if (ret) 
				...
				{
					sign_key_desc.pfs.obfuscated_key_id = PFS_FAKE_OBF_KEY_ID;
					sign_key_desc.pfs.key_size = sizeof(sign_key_desc.pfs.escrowed_key);

					GenPfsSignKey(ekpfs, hdr->cryptSeed, sign_key_desc.pfs.escrowed_key);

					fpu_kern_enter(td, fpu_kern_ctx, 0);
					{
						memset(iv, 0, sizeof(iv));
						ret = AesCbcCfb128Encrypt(sign_key_desc.pfs.escrowed_key, sign_key_desc.pfs.escrowed_key, sizeof(sign_key_desc.pfs.escrowed_key), g_FakeKeySeed, sizeof(g_FakeKeySeed) * 8, iv);
					}
					fpu_kern_leave(td, fpu_kern_ctx);
				}
				if (ret) 
...
int32_t OnNpdrmDecryptIsolatedRif(KeymgrPayload* p_Payload)
{
...
int32_t OnNpdrmDecryptRifNew(KeymgrPayload* p_Payload)
{
    if (p_Payload == nullptr)
    {
        WriteLog(LL_Error, "p_Payload is nullptr.");
...
int32_t OnSceSblKeymgrInvalidateKeySxXlock(struct sx* p_Sx, int p_Opts, const char* p_File, int p_Line) 
{
    if (p_Sx == nullptr)
    {
    ...
    TAILQ_FOREACH(keySlotDesc, sblKeymgrKeySlots, list)
    {
        ...
        if (keyDesc->desc.pfs.obfuscated_key_id != PFS_FAKE_OBF_KEY_ID) 
        {
        ...
        if (keyDesc->desc.pfs.key_size != sizeof(keyDesc->desc.pfs.escrowed_key)) 
        {
        ...
        memcpy(sblKeymgrBufVa, keyDesc->desc.pfs.escrowed_key, keyDesc->desc.pfs.key_size);
        //WriteLog(LL_Debug, "sblKeymgrBufGva %p %p", sblKeymgrBufGva, *sblKeymgrBufGva);
        ret2 = sceSblKeymgrSetKeyStorage(*sblKeymgrBufGva, keyDesc->desc.pfs.key_size, keyDesc->desc.pfs.obfuscated_key_id, keySlotDesc->keyId);
        if (ret2) 
        {
